...
Skills page (pinned by the product owner on 2026-09-06, replacing the Commands tab): a marketplace of every skill across the organisation's repositories. Pattern from Mobbin: Midday's integrations grid (search, All / Installed toggle, uniform cards each with Details and Install) https://mobbin.com/screens/b60de490-806e-4034-86e2-bcb97ff88880, with Kit's category list as repository chips https://mobbin.com/screens/428cb12b-0e79-46b5-8d39-672af35804e8. Data: the bundled catalogue at static/catalog/skills.json, generated by scripts/build_skill_catalog.py from GitHub at development time; the app never fetches. Each card: skill name, repository chip, one sentence; opening in place shows the slash command, the full description, an install prompt to copy (or to view first), and a square-with-arrow link to the repository on GitHub. The CLI command runner stays reachable from the page's foot and from quick actions.

Fix flow (pinned by the product owner on 2026-09-08: "extremely simple; for anything missing, a prompt to copy or open it in the terminal in one click; every option leads to that; green or yellow is good, a warning colour flags what needs them; badges"): every finding on the overview resolves to "Fix in Claude Code" (`mos open --prompt`, Claude Code opens in the brain's folder with the fix typed in) and "Copy the prompt", with "View the prompt" first if wanted. The header's primary is the fix-all when the next action is a repair or a skills sync. Preview-and-apply stays in the command runner. Status is three badges: green ready, Sun minor or optional, Ember Bright needs you, each a dot and a word. Unanswered questions keep the in-app interview as their action.
