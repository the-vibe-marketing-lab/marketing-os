...
- **`mos open --prompt`.** Passes text to Claude Code or Codex as its first message. On
  WSL, Windows and macOS the text goes into a launcher file under `~/.marketing-os/launch`
  and only that path reaches the terminal, because Windows Terminal splits its command
  line on `;` and `cmd.exe` rewrites `%` and newlines; on Linux it is one argv element.
